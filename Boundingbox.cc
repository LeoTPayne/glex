...
BoundingBox::BoundingBox(const Vector3 centre,
  ...
  extent_y(make_shared<Vector3>(Vector3(0.0, (float) height/2.0, 0.0))),
  extent_z(make_shared<Vector3>(Vector3(0.0, 0.0, (float) depth/2.0)))
 {
}
...
pair<float,float> BoundingBox::projectOntoAxis(const BoundingBox & b, enum AXIS axis) {
  ...
  case Y:
    ...
    case Z:
    {
      Vector3 by = *b.extent_z;
      lo  = projection( Point3(Vector3(*(b.centre)) + (by * -1)), zAxis());
      hi  = projection( Point3(Vector3(*(b.centre)) + by), zAxis());
    }
    break;
  }
...
ostream& operator<<(ostream& os, const BoundingBox& obj) {
  os << "c:(" << obj.centre->getX() << ", " << obj.centre->getY() << ") w:" << obj.centre->getZ() << ") d:" << (obj.extent_x->getX()*2) << " h:" << (obj.extent_y->getY()*2) << (obj.extent_z->getZ()*2);
  return os;
}
